...
{
    public class GetCustomerByIdHandler : BaseHandler<GetCustomerByIdHandler>
    {
        private ICustomerBusinessService service;
        private readonly ILogger<GetCustomerByIdHandler> logger;
        ...
        public GetCustomerByIdHandler(ILogger<GetCustomerByIdHandler> logger, IHttpClientFactory _httpClientFactory, IHttpContextAccessor accessor, IConfiguration config, AuthorizationHelper _authorizationHelper) : base(logger, _httpClientFactory, accessor, config, _authorizationHelper)
        {
            this.logger = logger;
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            try
            {
                //var authResponse = await _authorizationHelper.ValidateUserPermissionsAsync(Policy.Invoicing.DetailsCustomers);
                ...
                {
                    var response = new RestAPIGenericResponseDTO<CustomerResponseDto>().WithError(Constants.VALIDATION_ERROR_CODE, "Request URL is invalid");
                    var responseTxt = JSONHelper<RestAPIGenericResponseDTO<CustomerResponseDto>>.GetJSONStr(response);
