...
{
    public class UpdateInvoiceStatusHandler : BaseHandler<UpdateInvoiceStatusHandler>
    {
        private IInvoiceBusinessService service;
        private readonly ILogger<UpdateInvoiceStatusHandler> logger;
        ...
        public UpdateInvoiceStatusHandler(ILogger<UpdateInvoiceStatusHandler> logger, IHttpClientFactory _httpClientFactory, IHttpContextAccessor accessor, IConfiguration config, AuthorizationHelper _authorizationHelper) : base(logger, _httpClientFactory, accessor, config, _authorizationHelper)
        {
            this.logger = logger;
        ...
        {
            var authResponse = await _authorizationHelper.ValidateUserPermissionsAsync(Policy.Invoicing.ManageInvoicesCategories, Constants.UPDATE_INVOICE_SCOPE);
