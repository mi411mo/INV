...
namespace Application.ServiceImpl
{
    public class CategoryServiceImpl : ICategoryService
    {
        ...
        public async Task<ResultRepo<object>> AddAsync(Category category)
        {
            ...
            try
            {
               /* var customers = await categoryRepo.GetAllAsync();
                if (customers.Any(x => x.CategoryName == category.CategoryName))
                {
        ...
        public Task<ServiceResult<PagedListDto<Category>>> GetAllAsync(PaginationFilter filter, object oDataOptions)
        {
