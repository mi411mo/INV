...
{
    public class CreateInvoiceHandler : BaseHandler<CreateInvoiceHandler>
    {
        ...
        public CreateInvoiceHandler(InputValidationService inputValidationService,ILogger<CreateInvoiceHandler> logger, IHttpClientFactory _httpClientFactory, IHttpContextAccessor accessor, IConfiguration config, AuthorizationHelper _authorizationHelper) : base(logger, _httpClientFactory, accessor, config, _authorizationHelper)
        {
            this.logger = logger;
        ...
        {
            var response = await _authorizationHelper.ValidateUserPermissionsAsync(Policy.Invoicing.ManageInvoicesCategories, Constants.CREATE_INVOICE_SCOPE);
            ...
            var jSONValidationResponse = await inputValidationService.Validate<InvoiceRequestDto>(jSONstring);
            ...
            InvoiceRequestDto serviceRequest = await HTTPHelper<InvoiceRequestDto>.GetIsntance().GetBodyModel(request);
        ...
        private async void ReshapeRequest(HttpRequestMessage request)
        {
            ...
            var requestDto = JsonConvert.DeserializeObject<InvoiceRequestDto>(reqString);
