...
{
    public class CreateOrderHandler : BaseHandler<CreateOrderHandler>
    {
        ...
        public CreateOrderHandler(InputValidationService inputValidationService,ILogger<CreateOrderHandler> logger, IHttpClientFactory _httpClientFactory, IHttpContextAccessor accessor, IConfiguration config, AuthorizationHelper _authorizationHelper) : base(logger, _httpClientFactory, accessor, config, _authorizationHelper)
        {
            this.logger = logger;
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            var jSONValidationResponse = await inputValidationService.Validate<OrderRequestDto>(jSONstring);
            ...
            OrderRequestDto serviceRequest = await HTTPHelper<OrderRequestDto>.GetIsntance().GetBodyModel(request);
            ...
            var responseTxt = JSONHelper<RestAPIGenericResponseDTO<OrderResponseDto>>.GetJSONStr(serviceResponse);
        ...
        private async void ReshapeRequest(HttpRequestMessage request)
        {
            ...
            var requestDto = JsonConvert.DeserializeObject<OrderRequestDto>(reqString);
