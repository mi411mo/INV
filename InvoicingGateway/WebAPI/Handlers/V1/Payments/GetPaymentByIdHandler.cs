...
{
    public class GetPaymentByIdHandler : BaseHandler<GetPaymentByIdHandler>
    {
        ...
        public GetPaymentByIdHandler(ILogger<GetPaymentByIdHandler> logger, IHttpClientFactory _httpClientFactory, IHttpContextAccessor accessor, IConfiguration config, AuthorizationHelper _authorizationHelper) : base(logger, _httpClientFactory, accessor, config, _authorizationHelper)
        {
            this.logger = logger;
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            try
            {
               /* var authResponse = await _authorizationHelper.ValidateUserPermissionsAsync(Policy.Invoicing.DetailsPayments);
                ...
                if (request.RequestUri.Segments.Length == 4)
                {
                    ...
                    var serviceResponse = await service.GetPaymentById(id, base.SendAsync, request, cancellationToken);
