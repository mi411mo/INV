...
{
    public class ConfirmRequestPaymentHandler : BaseHandler<ConfirmRequestPaymentHandler>
    {
        private IRequestPaymentBusinessService service;
        private readonly ILogger<ConfirmRequestPaymentHandler> logger;
        ...
        public ConfirmRequestPaymentHandler(ILogger<ConfirmRequestPaymentHandler> logger, IHttpClientFactory _httpClientFactory, IHttpContextAccessor accessor, IConfiguration config, AuthorizationHelper _authorizationHelper) : base(logger, _httpClientFactory, accessor, config, _authorizationHelper)
        {
            this.logger = logger;
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            try
            {
                //var authResponse = await _authorizationHelper.ValidateUserPermissionsAsync(Policy.Invoicing.ManageInvoices);
                ...
                if (request.RequestUri.Segments.Length == 5)
                {
                    ...
                    var serviceResponse = await service.ConfirmRequestPayAsync(payRef, base.SendAsync, request, cancellationToken);

                    var responseTxt = JSONHelper<RestAPIGenericResponseDTO<RequestPaymentConfirmResponseDto>>.GetJSONStr(serviceResponse);
                ...
                {
                    var response = new RestAPIGenericResponseDTO<RequestPaymentConfirmResponseDto>().WithError(Constants.CLIENT_ERROR_CODE, "Request URL is invalid");
                    var responseTxt = JSONHelper<RestAPIGenericResponseDTO<RequestPaymentConfirmResponseDto>>.GetJSONStr(response);
