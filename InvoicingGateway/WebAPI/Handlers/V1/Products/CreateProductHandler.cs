...
namespace WebAPI.Handlers.V1.Products
{
    public class CreateProductHandler : BaseHandler<CreateProductHandler>
    {
        private IProductBusinessService service;
        ...
        public CreateProductHandler(InputValidationService inputValidationService,ILogger<CreateProductHandler> logger, IHttpClientFactory _httpClientFactory, IHttpContextAccessor accessor, IConfiguration config, AuthorizationHelper _authorizationHelper) : base(logger, _httpClientFactory, accessor, config, _authorizationHelper)
        {
            this.logger = logger;
        ...
        {
            var authResponse = await _authorizationHelper.ValidateUserPermissionsAsync(Policy.Invoicing.ManageProducts);

            if (authResponse != null)
            {
                return authResponse;
            }
            service = httpContextAccessor.HttpContext.RequestServices.GetRequiredService<IProductBusinessService>();

            var jSONstring = await request.Content.ReadAsStringAsync();
            var jSONValidationResponse = await inputValidationService.Validate<ProductRequestDto>(jSONstring);
            ...
            ProductRequestDto serviceRequest = await HTTPHelper<ProductRequestDto>.GetIsntance().GetBodyModel(request);
            ...
            var responseTxt = JSONHelper<RestAPIGenericResponseDTO<ProductResponseDto>>.GetJSONStr(serviceResponse);
        ...
        private async void ReshapeRequest(HttpRequestMessage request)
        {
            ...
            var requestDto = JsonConvert.DeserializeObject<ProductRequestDto>(reqString);
