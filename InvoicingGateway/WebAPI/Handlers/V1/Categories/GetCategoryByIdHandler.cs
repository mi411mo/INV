...
{
    public class GetCategoryByIdHandler : BaseHandler<GetCategoryByIdHandler>
    {
        private ICategoryBusinessService service;
        private readonly ILogger<GetCategoryByIdHandler> logger;
        ...
        public GetCategoryByIdHandler(InputValidationService inputValidationService,ILogger<GetCategoryByIdHandler> logger, IHttpClientFactory _httpClientFactory, IHttpContextAccessor accessor, IConfiguration config, AuthorizationHelper _authorizationHelper) : base(logger, _httpClientFactory, accessor, config, _authorizationHelper)
        {
            this.logger = logger;
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            try
            {
                //var authResponse = await _authorizationHelper.ValidateUserPermissionsAsync(Policy.Invoicing.DetailsInvoicesCategories);

                //if (authResponse != null)
                //{
                //    return authResponse;
                //}
                service = httpContextAccessor.HttpContext.RequestServices.GetRequiredService<ICategoryBusinessService>();
                ...
                {
                    var response = new RestAPIGenericResponseDTO<CategoryResponseDto>().WithError(Constants.VALIDATION_ERROR_CODE, "Request URL is invalid");
                    var responseTxt = JSONHelper<RestAPIGenericResponseDTO<CategoryResponseDto>>.GetJSONStr(response);
