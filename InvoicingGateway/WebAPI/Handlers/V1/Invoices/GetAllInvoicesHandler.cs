...
{
    public class GetAllInvoicesHandler : BaseHandler<GetAllInvoicesHandler>
    {
        private IInvoiceBusinessService service;
        private readonly ILogger<GetAllInvoicesHandler> logger;
        ...
        public GetAllInvoicesHandler(ILogger<GetAllInvoicesHandler> logger, IHttpClientFactory _httpClientFactory, IHttpContextAccessor accessor, IConfiguration config, AuthorizationHelper _authorizationHelper) : base(logger, _httpClientFactory, accessor, config, _authorizationHelper)
        {
            this.logger = logger;
        ...
        {
            var response = await _authorizationHelper.ValidateUserPermissionsAsync(Policy.Invoicing.ManageInvoicesCategories, Constants.GETALL_INVOICE_SCOPE);
