...
{
    public class UpdateCustomerHandler : BaseHandler<UpdateCustomerHandler>
    {
        ...
        public UpdateCustomerHandler(ILogger<UpdateCustomerHandler> logger, IHttpClientFactory _httpClientFactory, IHttpContextAccessor accessor, IConfiguration config, AuthorizationHelper _authorizationHelper) : base(logger, _httpClientFactory, accessor, config, _authorizationHelper)
        {
            this.logger = logger;
        ...
        {
            var authResponse = await _authorizationHelper.ValidateUserPermissionsAsync(Policy.Invoicing.ManageCustomers);
