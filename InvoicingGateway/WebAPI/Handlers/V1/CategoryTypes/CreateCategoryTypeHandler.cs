...
{
    public class CreateCategoryTypeHandler : BaseHandler<CreateCategoryTypeHandler>
    {
        private ICategoryTypeBusinessService service;
        private readonly ILogger<CreateCategoryTypeHandler> logger;
        ...
        public CreateCategoryTypeHandler(InputValidationService inputValidationService,ILogger<CreateCategoryTypeHandler> logger, IHttpClientFactory _httpClientFactory, IHttpContextAccessor accessor, IConfiguration config, AuthorizationHelper _authorizationHelper) : base(logger, _httpClientFactory, accessor, config, _authorizationHelper)
        {
            this.logger = logger;
        ...
        {
            var response = await _authorizationHelper.ValidateUserPermissionsAsync(Policy.Invoicing.ManageCategoryTypes);
            ...
            var jSONValidationResponse = await inputValidationService.Validate<CategoryTypeRequestDto>(jSONstring);
            ...
            CategoryTypeRequestDto serviceRequest = await HTTPHelper<CategoryTypeRequestDto>.GetIsntance().GetBodyModel(request);
        ...
        private async void ReshapeRequest(HttpRequestMessage request)
        {
            ...
            var requestDto = JsonConvert.DeserializeObject<CategoryTypeRequestDto>(reqString);
