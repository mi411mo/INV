...
{
    public class GetInvoiceByReferenceHandler : BaseHandler<GetInvoiceByReferenceHandler>
    {
        private IInvoiceBusinessService service;
        private readonly ILogger<GetInvoiceByReferenceHandler> logger;
        ...
        public GetInvoiceByReferenceHandler(ILogger<GetInvoiceByReferenceHandler> logger, IHttpClientFactory _httpClientFactory, IHttpContextAccessor accessor, IConfiguration config, AuthorizationHelper _authorizationHelper) : base(logger, _httpClientFactory, accessor, config, _authorizationHelper)
        {
            this.logger = logger;
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            try
            {
                var authResponse = await _authorizationHelper.ValidateUserPermissionsAsync(Policy.Invoicing.ManageInvoicesCategories, Constants.GETBYREFERENCE_INVOICE_SCOPE);
                ...
                if (request.RequestUri.Segments.Length == 4)
                {
                    ...
                    var responseTxt = JSONHelper<RestAPIGenericResponseDTO<InvoiceResponseDto>>.GetJSONStr(serviceResponse);
                ...
                {
                    var response = new RestAPIGenericResponseDTO<InvoiceResponseDto>().WithError(4000, "Request URL is invalid");
                    var responseTxt = JSONHelper<RestAPIGenericResponseDTO<InvoiceResponseDto>>.GetJSONStr(response);
