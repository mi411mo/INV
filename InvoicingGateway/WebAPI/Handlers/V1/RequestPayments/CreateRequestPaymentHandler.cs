...
{
    public class CreateRequestPaymentHandler : BaseHandler<CreateRequestPaymentHandler>
    {
        private IRequestPaymentBusinessService service;
        private readonly ILogger<CreateRequestPaymentHandler> logger;
        ...
        public CreateRequestPaymentHandler(InputValidationService inputValidationService,ILogger<CreateRequestPaymentHandler> logger, IHttpClientFactory _httpClientFactory, IHttpContextAccessor accessor, IConfiguration config, AuthorizationHelper _authorizationHelper) : base(logger, _httpClientFactory, accessor, config, _authorizationHelper)
        {
            this.logger = logger;
        ...
        {
            var response = await _authorizationHelper.ValidateUserPermissionsAsync(Policy.Invoicing.ManageInvoices);
            ...
            var jSONValidationResponse = await inputValidationService.Validate<RequestPaymentRequestDto>(jSONstring);
            ...
            RequestPaymentRequestDto serviceRequest = await HTTPHelper<RequestPaymentRequestDto>.GetIsntance().GetBodyModel(request);
        ...
        private async void ReshapeRequest(HttpRequestMessage request)
        {
            ...
            var requestDto = JsonConvert.DeserializeObject<RequestPaymentRequestDto>(reqString);
